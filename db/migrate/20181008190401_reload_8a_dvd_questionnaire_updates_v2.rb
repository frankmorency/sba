class Reload8aDvdQuestionnaireUpdatesV2 < ActiveRecord::Migration
  def change
    Questionnaire.get('eight_a_disadvantaged_individual').update_attribute :name, 'eight_a_disadvantaged_individual_v_three'
    ...
    SectionRule.where(to_section_id: Questionnaire.get('eight_a_disadvantaged_individual').sections.where(name: 'review').pluck(:id)).each do |rule|
