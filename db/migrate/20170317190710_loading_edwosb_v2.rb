class LoadingEdwosbV2 < ActiveRecord::Migration
  def change
    
    c = QuestionType::Currency.new name: 'currency_with_comment_required_on_positive_value', title: 'Currency'
    c.question_rules.new mandatory: true, capability: :add_comment, condition: :equal_to, value: '0'
    c.save!

    Questionnaire.get('edwosb').update_attribute :name, 'edwosb_v_two'
    ...
    # Updating all the is_last flags
    ...
    Section.where(questionnaire_id: Questionnaire.get('edwosb').id, name: 'signature').update_all is_last: true
  end
end
