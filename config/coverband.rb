if Rails.env.qa_admin? || Rails.env.qa? || Rails.env.development?
  Coverband.configure do |config|
    config.root = Dir.pwd

#     if defined? Redis
       #config.redis = Redis.new(host: 'localhost', port: 49_182, db: 1)
...
#     config.root_paths = ['/app/'] # /app/ is needed for heroku deployments
    ...
    config.ignore = ['vendor'
                    #,'lib/scrazy_i18n_patch_thats_hit_all_the_time.rb'
                    ]
